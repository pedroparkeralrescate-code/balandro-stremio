...
def get_servers_list():
    ...
    for server in os.listdir(os.path.join(config.get_runtime_path(), 'servers')):
        if server.endswith('.json'):
            ...
            # Verificar que el servidor tenga 'id' antes de comparar
            if not server_parameters or 'id' not in server_parameters:
                logger.error('El servidor %s no tiene un campo "id" o está vacío.' % serverid)
                continue
...
def corregir_other(srv):
    ...
    elif 'lulustream' in srv or 'luluvdo' in srv or 'streamhihi' in srv or 'lulustream' in srv or 'luluvdoo' in srv or 'lulu' in srv or 'ponmi' in srv or 'd00ds.site' in srv: srv = 'Lulustream'
