...
# Versiones Firefox
cod_fver = [60, 66, 68, 70, 72, 73, 74, 76, 77, 78, 81, 82, 83, 84]
fver = random.choice(cod_fver)

ff_versions = [78, 79, 80, 81, 82, 83, 84, 85, 86, 87, 88, 89]  # (Windows NT 10.0; Win64; x64; rv:%s.0)
...
def obtener_last_chrome():
    ...
    if ver_stable_chrome:
        ...
        if not cfg_last_ver_chrome == '':
            last_chrome = cfg_last_ver_chrome
    else:
        try:
...
def do_downloadpage(url, headers = None):
    data = httptools.downloadpage_proxy(canal, url = url, headers = headers).data
    return data
...
def series(item):
    ...
    if next_page_link:
       itemlist.append(item.clone ( title = '>> Pagina siguiente', action = 'series', url = next_page_link, referer = next_page_link, text_color='coral' ))
...
def list_all(item):
    ...
    if not next_page_link:
        if '/newvideos.html?d=month' in item.url or '/search.php?keywords=' in item.url:
    ...
    if next_page_link: itemlist.append(item.clone ( title = '>> Pagina siguiente', action = 'list_all', url = next_page_link, referer = next_page_link ))
    else:
        if item.page != '':
            if num_matches > hasta: itemlist.append(item.clone( title='>> Pagina siguiente', page = item.page + 1, action = 'list_all', text_color='coral' ))
    ...
    if 'lo sentimos este documental ha sido eliminado' in data.lower():
        ...
        if ' de mantenimiento' in data.lower() or ' retornar a la normalidad' in data.lower():
            if notification_d_ok:
                platformtools.dialog_ok(cnomv, 'Canal en Mantenimiento')
            else:
                platformtools.dialog_notification(cnomv, '[COLOR moccasin]Canal en Mantenimiento[/COLOR]')
            return
    ...
    if not url_embed:
        ...
        if not url_embed:
            if '/age-verification.js' in data:
                if descartar_xxx: platformtools.dialog_notification(cnomv, '[COLOR moccasin]Temática para adultos[/COLOR]')
                return
