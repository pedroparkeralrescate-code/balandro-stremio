...
def do_downloadpage(url, post=None, headers=None, raise_weberror=True):
    ...
    if '/ver/' in url or '/series' in url:
...
def last_epis(item):
    ...
    if num_matches > perpage:
        ...
        if hasta < num_matches:
            itemlist.append(item.clone( title='>> Página siguiente', page=item.page + 1, action='last_epis', text_color='coral' ))
            buscar_next = False
    ...
    if buscar_next:
        ...
        if next_page:
            if '/page/' in next_page:
                itemlist.append(item.clone (url = next_page, page = 0, title = '>> Página siguiente', action = 'last_epis', text_color='coral' ))
